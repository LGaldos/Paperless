...
function showStepAreas(id1, id2, id3, class1, class2) {
    ...
    image.src = "/_layouts/15/ESMA.Paperless.Design.v15/images/RSMinus.gif";
    image.alt = "Collapse";
}
...
function toggleAreas(id1, id2, id3, class1, class2) {
    ...
    if (area.style.display.toUpperCase() == "BLOCK") {
        ...
        image.src = "/_layouts/15/ESMA.Paperless.Design.v15/images/RSPlus.gif";
        image.alt = "Expand";
    }
    else {
        setCookie(cookieName, "BLOCK", 365);
    ...
    if (arrFiles.length > 0) {
        ...
        for (a = 0; a < objView.length; a++) {
            if (objView[a].getElementsByTagName("img").length > 0) {
                ...
                objView[a].innerHTML = '<a id="CallOutExample" onMouseOver="OpenItemFilePreviewCallOut(this, ' + "\'" + files[1] + "\'," + "\'" + officeServer + "/_layouts/15/WopiFrame2.aspx?sourcedoc=" + encodeURI(files[0]) + '\')" title="Preview Document" href="#"><img src="/_layouts/15/ESMA.Paperless.Design.v15/images/RSPreview.png" alt=""/></a>';
                cont++;
            }
        }
    }
