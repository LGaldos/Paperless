...
namespace ESMA.Paperless.Webparts.v15.RSWorkflowViewAll
{
    public partial class RSWorkflowViewAllUserControl : UserControl
    {
        ...
        const string URL_IMG_CONFIDENTIAL = "/_layouts/15/ESMA.Paperless.Design.v15/images/RSConfidential.gif";
                ...
                using (SPSite Site = new SPSite(SPContext.Current.Web.Url.ToString()))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        foreach (SPListItem item in collWF)
                        {
                            ...
                            {
                                SaveErrorsLog("GetDataWorkflows() ViewAll - WFID: '" + WFID + "' not showed.", null);
                                continue;
                            }
        ...
        public static Dictionary<string, string> GetConfigurationParameters(SPWeb Web)
        {
            ...
            try
            {
                ...
                foreach (SPListItem item in itemCollection)
                {
                    ...
                    {
                        if (item["Value1"] != null)
                            parameters.Add(item.Title, item["Value1"].ToString());
                    }
                    catch { continue; }
                }
            }
